...
if __name__ == "__main__":   
    ...
    ax1.set_xlabel('x (m)')
    ax1.set_ylabel('t (s)')
    ax1.set_title('Propagation sech soliton with confining potential',size=16)
    ax2.set_xlabel('x (m)')
    ax2.set_ylabel('t (s)')
    ax3.xaxis.set_ticklabels([])
    #ax2.set_ylabel(r'N = $\int_a^b |\psi(x,t)|^2 dx$')
    ax2.set_title('Propagation sech soliton with non-negative potential',size=16)
    ax3.set_xlabel('x',size=14)
    ax3.set_ylabel('$|\psi(x,t)|^2$', size=14)
    myAxes=(ax1,ax2,ax3)
